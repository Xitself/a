            ...
            local success, result = pcall(function()
                repeat
                    ...
                    if not gunDrop then
                        ...
                        local distance = (murderer.Character.HumanoidRootPart.Position - gunDrop.Position).Magnitude

                        if distance > 10 then
                            if not GettingGun then
                                local savedPosition = Humanoid.CFrame
                                GettingGun = true
                                Humanoid.CFrame = CFrame.new(gunDrop.Position + Vector3.new(0, 5, 0))
